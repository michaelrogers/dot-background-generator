...
var skewDegrees = 7; skewDegrees /= 100;
...
function timeoutResize(){
	...
	timeoutResize = setTimeout(function(){resizeCanvas()},200);
}

function resizeCanvas(){

	console.count("resizeCanvas");
...
function dotsLoop () {
	...
	decrementVariable = 105 * (dotCenterDistance)/ window.innerHeight; //Determine what percentage to decrement each turn
	...
	for (let x=0; x < (window.innerWidth / Math.sin(skewDegrees)) / (dotCenterDistance); x++ ){ //The hypotenuse is the is the length of the furtherst width and is the width of the window / sin(skewDegrees)
		for (let y=0; y < Math.ceil(window.innerHeight * Math.tan(skewDegrees)); y++){ 
			...
			if (y % 2 == 0){dotColor = color1; oddRowSpacing = (dotCenterDistance - dotMaxRadius)}
